...
// OVOZ MEXANIZMI
window.playExp = function() {
    if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const mainGain = audioCtx.createGain();
    mainGain.gain.setValueAtTime(0, audioCtx.currentTime);
    mainGain.gain.linearRampToValueAtTime(0.5, audioCtx.currentTime + 0.01);
    mainGain.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + 1.5);

    const boom = audioCtx.createOscillator();
    boom.type = 'triangle';
    boom.frequency.setValueAtTime(60, audioCtx.currentTime);
    boom.frequency.exponentialRampToValueAtTime(5, audioCtx.currentTime + 0.4);
    
    boom.connect(mainGain);
...
async function handleClaim() {
    if (!AdController) {
        window.Telegram.WebApp.showAlert("Ad failed to load.");
        return;
    }
        ...
        if (result && result.done) {
            ...
            if (snapshot.exists()) {
                ...
                if (now - data.lastClaim < 5 * 60 * 1000) { 
                    window.Telegram.WebApp.showAlert("Please wait!"); return; 
                }
                await update(userRef, { balance: (data.balance || 0) + reward, lastClaim: now });

                if (data.invitedBy) {
                    const bossRef = ref(db, 'users/' + data.invitedBy);
                    const bossSnap = await get(bossRef);
                    if (bossSnap.exists()) {
                        await update(bossRef, { balance: (bossSnap.val().balance || 0) + (reward * bonusPercent) });
                    }
                }
            } else {
                await set(userRef, { balance: reward, lastClaim: now, invitedBy: referrerId });
            }
            loadUserData();
        }
...
async function loadUserData() {
    ...
    if (snapshot.exists()) {
        const data = snapshot.val();
        if (data.isBlocked) { document.body.innerHTML = ""; return; }
